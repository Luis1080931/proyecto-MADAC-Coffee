...
export const CrearMuestra = async (req, res) => {
    ...
    if (!errors.isEmpty()) {
        ...
        const { fecha, cantidad, proceso_fermentacion, altura_MSNM, tiempo_fermentacion, tiempo_secado,presentacion, fk_lote } = req.body;
        const [resultado] = await pool.query("INSERT INTO muestras (fecha, cantidad, proceso_fermentacion, altura_MSNM, tiempo_fermentacion, tiempo_secado,presentacion, fk_lote, estado) VALUES (?, ?, ?, ?, ?, ?, ?, ?, 1)", [fecha, cantidad, proceso_fermentacion, altura_MSNM, tiempo_fermentacion, tiempo_secado, presentacion, fk_lote]);
...
export const actualizarMuestra = async (req, res) => {
    try {
        ...
        const { fecha, cantidad, /* densidad_cafe */ proceso_fermentacion, /* tipo_tostion */ altura_MSNM, tiempo_fermentacion, /* actividad_agua */ tiempo_secado,presentacion, fk_lote } = req.body;

        const [result] = await pool.query('UPDATE muestras SET fecha = IFNULL(?, fecha), cantidad = IFNULL(?, cantidad), proceso_fermentacion = IFNULL(?, proceso_fermentacion) altura_MSNM = IFNULL(?, altura_MSNM), tiempo_fermentacion = IFNULL(?, tiempo_fermentacion), tiempo_secado = IFNULL(?, tiempo_secado), presentacion = IFNULL(?, presentacion) , fk_lote = IFNULL(?, fk_lote), estado=1 WHERE codigo = ?', [fecha, cantidad, /* densidad_cafe */, proceso_fermentacion, /* tipo_tostion */, altura_MSNM, tiempo_fermentacion, /* actividad_agua */, tiempo_secado, presentacion, fk_lote, codigo]);
