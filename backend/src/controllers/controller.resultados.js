...
export const listarResultados = async (req, res) => {
  try {
    const sql = 'SELECT codigo, fecha, fk_analisis AS analisis, nombre AS variable, observaciones, valor, r.estado FROM resultados AS r JOIN variables ON fk_variables = v_codigo';
...
export const registrarResultados = async (req, res) => {
  try {
    ...
    const {
      fecha, fk_analisis, fk_variables, valor, observaciones, estado,
    } = req.body;

    const sql = 'INSERT INTO resultados (fecha, fk_analisis, fk_variables, observaciones, valor, estado) values (?, ?, ?, ?, ?, ?)';
...
export const actualizarResultado = async (req, res) => {
  try {
    ...
    const {
      fecha, fk_analisis, fk_variable, observaciones, valor,
    } = req.body;
    const { id } = req.params;
...
export const desactivarResultado = async (req, res) => {
  try {
    ...
    const sql = 'UPDATE resultados SET estado = 2 WHERE codigo = ?';
