...
function Variedad() {
  ...
  const fetchData = async () => {
    try {
      ...
      const baseURL = 'http://10.193.144.95:3000/listarVariedades';
  ...
  const handleActualizacionFormSubmit = async (formData) => {
    try {
      ...
      const response = await axios.put(`http://10.193.144.95:3000/actualizarVariedad/${id_variedad}`, formData, {
        headers: {
          'token': token
        }
      });
  ...
  const handleSearch = async (searchTerm) => {
    try {
      ...
      if (searchTerm.trim() === '') {
        ...
        const response = await axios.get(`http://10.193.144.95:3000/buscarVariedad/${searchTerm}`, {
          headers: {
            'token': token
          }
        });
  ...
  const handleEstadoBotonClick = async (id, estado) => {
    try {
      ...
      const response = await axios.put(`http://10.193.144.95:3000/desactivar/Variedad/${id}`, { estado: newEstado }, {
        headers: {
          'token': token
        }
      });
