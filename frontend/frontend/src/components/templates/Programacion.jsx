...
function Programacion() {
  ...
  const fetchData = async () => {
    try {
      ...
      const baseURL = 'http://10.193.144.95:3000/listarProgramacion';
  ...
  const handleActualizacionFormSubmit = async (formData) => {
    try {
      ...
      const { id } = formData;
      const response = await axios.put(`http://10.193.144.95:3000/actualizarProgramacion/${id}`, formData, {
        headers: {
          'token': token
        }
      });
  ...
  const handleSearch = async (searchTerm) => {
    try {
      ...
      if (searchTerm.trim() === '') {
        ...
        const response = await axios.get(`http://10.193.144.95:3000/buscarProgramacion/${searchTerm}`, {
          headers: {
            'token': token
          }
        });
    ...
    try {
      ...
      if (initialData) {
        ...
        if (result.isConfirmed) {
          ...
          await axios.put(`http://10.193.144.95:3000/desactivar/Programacion/${id_programacion}`, { estado: formData.estado }, {
            headers: {
              'token': token
            }
          });
