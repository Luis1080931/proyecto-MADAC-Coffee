...
export function Muestras() {
        ...
        case "actions":
          return (
            <div className="relative flex justify-end items-center gap-2">
              <FaEye
                className="cursor-pointer text-3xl text-black mr-5"
                onClick={() => ver(muestra.codigo)}
              />
              <ButtonActualizar
                click={() => handleToggle("update", setMuestrasId(muestra))}
              />
              {muestra.estado === "activo" ? (
                <ButtonDesactivar
                  click={() => handleDesactivar(muestra.codigo)}
                />
              ) : (
                <ButtonActivar click={() => handleActivar(muestra.codigo)} />
              )}
            </div>
          );
        default:
          ...
          <div className="flex flex-col gap-4">
            <div className="flex justify-between gap-3 items-end">
              ...
              <div className="flex gap-3">
                ...
                <Button
                  className="text-xl bg-[#273468] text-white"
                  endContent={<PlusIcon />}
            ...
            <div className="flex justify-between items-center">
              <span className="text-default-400 text-xl">
                Total {muestras.length} Resultados
              </span>
    ...
    return (
      <div className="flex items-center justify-center">
        ...
        >
          ...
          <TableBody
            emptyContent={"No hay muestras registradas"}
            items={sortedItems}
          >
            {(item) => (
  ...
  const { idMuestras, setMuestrasId, muestra, getMuestra } = useContext(
    MuestrasContext
  );
  ...
  const handleDesactivar = (codigo) => {
    try {
      axiosClient
        .put(`/muestras/desactivarMuestra/${codigo}`, null)
        .then((response) => {
          console.log(response.data);
  ...
  const handleSubmit = async (datosForm, e) => {
    ...
    try {
      if (mode === "create") {
        await axiosClient
          .post("/muestras/crearMuestra", datosForm)
          .then((response) => {
            console.log(response.data);
            if (response.status == 200) {
              setMensaje(response.data.message);
        ...
        await axiosClient
          .put(`/muestras/actualizarMuestra/${idMuestras.codigo}`, datosForm)
          .then((response) => {
            console.log(response.data);
        ...
        <div className="bg-[#EAEDF6]">
          <div className="w-full max-w-[90%] ml-28 items-center p-10">
            ...
            <MuestrasModal
              ...
              title={
                mode === "create" ? "Registrar Muestra" : "Actualizar Muestra"
              }
            ...
            <VerMuestras
              ...
              data={datosMuestras}
            />
            <Ejemplo data={data} muestras={muestras} />
          </div>
        </div>
      </div>
    </div>
